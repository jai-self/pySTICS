...
def compute_outputs_day_zero(outputs, crop, soil, manage, constants, initial, station, lracz, hur, wi, esz, epz, tsol, humirac, humpotsol, psisol, racinepsi, amplz, ndebsen, hurlim, aevap, s, fco2s, durviei, gamma, msresiduel, fco2):
    ...
    outputs['dayLAIcreation'], outputs['durage'], outputs['senstress'], outputs['tdevelop'], outputs['durvie'], outputs.loc[0,'dltaisen'], outputs.loc[0,'somsenreste'], ndebsen, outputs.loc[0,'somtemp'], outputs.loc[0,'dltamsen'], outputs.loc[0,'deltamsresen'], outputs.loc[0,'msres'], outputs.loc[0,'msresjaune'], outputs.loc[0,'durvie_n'] = senescence(0, outputs['lev'], outputs.loc[0,'ulai'], outputs.loc[0,'somtemp'], crop.VLAIMAX, durviei, crop.DURVIEF, 1, outputs.loc[0,'fstressgel'],
            outputs['dayLAIcreation'].array, outputs['senstress'].array, outputs['tdevelop'].array, outputs['durvie'].array, outputs['durage'].array, outputs['deltai'].array, outputs.loc[0,'somsenreste'], initial.LAI0, ndebsen, outputs['dltafv'], crop.RATIOSEN, crop.CODEPLANTE, outputs.loc[0,'msres'], msresiduel, outputs.loc[0,'msresjaune'], crop.CODLAINET, outputs['pfeuilverte'], outputs['dltams'], outputs.loc[0,'dltaisen'])
    ...
    ### Root growth ###
    ###################
    outputs.loc[0,'zrac'], outputs.loc[0,'deltaz'], outputs.loc[0,'deltaz_t'], outputs.loc[0,'deltaz_stress'], outputs.loc[0,'efda'], outputs.loc[0,'znonli'] = root_growth(outputs.loc[0,'ger'], outputs.loc[0,'lax'], outputs.loc[0,'sen'], outputs.loc[0,'rec'], crop.STOPRAC, crop.CODEPERENNE, outputs.loc[0,'findorm'], manage.PROFSEM, outputs.loc[0,'zrac'], crop.CODETEMPRAC,
    ...
    ### Root density ###
    ####################
    lracz[0], outputs.loc[0,'cumlracz'], outputs.loc[0,'zdemi'], humirac[0], outputs.loc[0,'humirac_mean'] = root_density(lracz[0], outputs.loc[0,'zrac'], outputs.loc[0,'znonli'], soil.DEPTH, crop.ZPRLIM, crop.ZPENTE, outputs.loc[0,'ger'], crop.CODEPERENNE, constants.LVOPT, s, manage.PROFSEM, 
    ...
    ### Water balance #####
    #######################
    hur[0], outputs.loc[0,'drain'] = water_balance(outputs.loc[0,'trr'], outputs.loc[0,'airg'], outputs.loc[0,'mouill'], soil.DEPTH, hur[0], np.array([0 for i in range(len(outputs))]), esz[0], soil.HCC, hurlim)
    ...
    epz[0] = soil_contribution_to_transpiration(outputs.loc[0,'ep'], epz[0], soil.DEPTH, hur[0], soil.HMIN, lracz[0], outputs.loc[0,'lev'], manage.PROFSEM, outputs.loc[0,'zrac'])
    ...
    ###################################################################
    outputs.loc[0,'rnet'], outputs.loc[0,'rglo'], outputs.loc[0,'albedolai'], outputs.loc[0,'albsol'], outputs.loc[0,'tcult'], outputs.loc[0,'tcultmax'], outputs.loc[0,'converge'], outputs.loc[0,'tcultmin'] = iterative_calculation(outputs.loc[0,'tmoy'], outputs.loc[0,'lev'], outputs.loc[0,'temp_max'], outputs.loc[0,'temp_min'], outputs.loc[0,'et'], outputs.loc[0,'z0'], soil.ALBEDO, hur[0,0], soil.HMINF_1, soil.HCCF_1, station.ALBVEG, outputs.loc[0,'lai'], outputs.loc[0,'trg'], outputs.loc[0,'tpm'], outputs.loc[0,'fracinsol'], station.CODERNET,
